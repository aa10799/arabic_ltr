...
// Define the number of trials to randomly select
    const numRandomTrials = 60;

// Randomly select 60 trials
    const practiceStimuli = jsPsych.randomization.sampleWithoutReplacement(stimuli_variables, numRandomTrials);

// Define the remaining trials (excluding the randomly selected ones)
...
var separatorMessage = {
    ...
    stimulus: `<p>Now, it's time for the main experiment.</p>
        <p>You will go through a total of 7 blocks with short breaks in between.</p>
...
var endMessage = {
  ...
  stimulus: `<p>You're done with the experiment.</p>
    ...
    // Debrief block after each chunk of 100 trials
    var chunk_debrief_block = {
        ...
        stimulus: function () {
            ...
            return `<p>Block ${block_number} accuracy: ${chunk_accuracy}%.</p>
                    <p>Average response time for the last 100 trials: ${chunk_rt}ms.</p>
    ...
    timeline: [
        fixation, 
        {
            ...
            trial_duration: function () {
                // Calculate trial_duration based on the current block number
                var block_number = Math.ceil(jsPsych.data.get().filter({ task: 'response' }).count() / 100);
                var trial_duration;

                if (block_number <= 6) {
                    // Adjust trial_duration for each block
                    trial_duration = 50 + (block_number - 1) * 10;
                } else {
                    // Default trial_duration if block_number exceeds 6
                    trial_duration = 100;
                }

                // Record block_number and trial_duration in the data
    ...
    randomize_order: true,
    repetitions: 1, // Run through all 600 trials once
